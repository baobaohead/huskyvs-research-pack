...
import src.forward_simulation_v5_1_7 as sim  # noqa: E402

from src.forward_simulation_v5_1_7 import (  # noqa: E402
    DEMO,
    FixtureAdapter,
    STRATEGY_IDS,
    audit_integrity,
...
from src.polymarket_public_adapter_v5_1_7 import (  # noqa: E402
    AdapterError,
    PublicAdapter,
    calculate_fee,
    clob_token_pairs,
...
def copy_v514_release_files(dst: Path) -> Path:
    for rel in [
        "src/forward_simulation_v5_1_7.py",
        "src/forward_reporting_v5_1_7.py",
        "src/polymarket_public_adapter_v5_1_7.py",
        "schemas/forward_simulation_v5_1_7.sql",
        "config/forward_simulation_v5_1_7.yaml",
    ]:
        target = dst / rel
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(PROJECT_ROOT / rel, target)
    for rel, body in {
        "reports/FORWARD_SIMULATION_V5_1_7_PREREGISTRATION.md": "preregistration\n",
        "reports/FORWARD_SIMULATION_V5_1_7_API_CONTRACT.md": "api contract\n",
        "reports/FORWARD_SIMULATION_V5_1_7_FEE_CONTRACT.md": "fee contract\n",
        "reports/FORWARD_SIMULATION_V5_1_7_SETTLEMENT_FINALITY_CONTRACT.md": "settlement finality contract\n",
    }.items():
        target = dst / rel
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(body, encoding="utf-8")
    return dst / "config/forward_simulation_v5_1_7.yaml"
...
def test_deleted_frozen_file_refuses_formal_write(tmp_path):
    ...
    (tmp_path / "reports/FORWARD_SIMULATION_V5_1_7_API_CONTRACT.md").unlink()
...
def test_resolved_raw_evidence_files_hash_recomputable(tmp_path):
    ...
    raw_dir = tmp_path / "data/forward_v5_1_7/demo/resolved_market_raw"
...
def test_rc6_infinite_loop_sleeps_between_three_rounds(monkeypatch, tmp_path):
    calls = []
    sleeps = []
...
def test_rc6_finite_loop_sleep_counts(monkeypatch, tmp_path):
    calls = []
    sleeps = []
...
def test_rc6_single_iteration_does_not_sleep(monkeypatch, tmp_path):
    calls = []
    sleeps = []
    monkeypatch.setattr(sim, "_monitor_once_under_lock", lambda *a, **k: calls.append(1) or {"run_id": a[3]})
...
def test_rc6_pause_uses_low_frequency_wait_and_no_monitor(monkeypatch, tmp_path):
    init_ledger(tmp_path, DEMO, CONFIG)
...
def test_rc6_resume_after_pause_enters_normal_loop(monkeypatch, tmp_path):
    init_ledger(tmp_path, DEMO, CONFIG)
    monitor_control(tmp_path, DEMO, CONFIG, "pause")
    calls = []
    sleeps = []
...
def test_rc6_fatal_and_recoverable_errors(monkeypatch, tmp_path):
    sleeps = []
...
def test_rc6_ctrl_c_releases_lock_and_marks_stopped(monkeypatch, tmp_path):
    sleeps = []
...
def test_rc6_monitor_once_uses_same_lock_and_rejects_second_instance(tmp_path):
    config = load_config(CONFIG)
    init_ledger(tmp_path, DEMO, CONFIG)
...
def test_rc6_formal_rejects_fixture_and_custom_adapter(tmp_path):
    init_ledger(tmp_path, "formal", CONFIG)
...
def test_rc6_gamma_real_tick_min_fields_are_read_and_crosschecked():
...
def test_rc6_only_gamma_constraints_do_not_allow_formal_orderbook():
...
def test_rc6_proposed_and_auto_resolved_proposed_never_final():
...
def test_rc6_winning_asset_is_strong_final_evidence():
...
def test_rc6_clob_public_token_winner_is_strong_final_evidence():
...
def test_rc6_beijing_question_city_parses_without_temperature_suffix():
...
def test_rc6_second_city_question_parses():
...
def test_rc6_temperature_bucket_regressions():
    expected = {
        "30C": "exact:30C",
        "20C": "exact:20C",
        "10C": "exact:10C",
...
def test_rc6_exact_or_below_or_higher_are_distinct():
...
def test_rc6_real_beijing_exact_signal_passes_and_boundary_rejects():
...
def test_rc6_beijing_or_below_signal_passes_and_exact_rejects():
...
def test_rc6_accepting_orders_missing_and_conflict_fail_closed():
    missing = market()
...
def test_rc6_lock_recovery_active_pid_stale_is_not_recoverable():
    config = load_config(CONFIG)
    info = {
        "pid": sim.os.getpid(),
...
def test_rc6_lock_recovery_dead_pid_is_recoverable():
    config = load_config(CONFIG)
    info = {
        "pid": 999999,
        "hostname": sim.socket.gethostname(),
...
def test_rc6_internal_monitor_requires_valid_lock_token(tmp_path):
    with pytest.raises(RuntimeError):
        sim._monitor_once_under_lock(tmp_path, DEMO, CONFIG, "no_token", None)


def test_rc6_forged_and_old_lock_tokens_rejected(tmp_path):
    config = load_config(CONFIG)
...
def test_rc6_public_monitor_rejects_reentry_while_lock_held(tmp_path):
    config = load_config(CONFIG)
...
def test_rc6_audit_recomputes_gamma_and_clob_hashes(tmp_path):
    demo_run(tmp_path, CONFIG)
    config = load_config(CONFIG)
    conn = connect(db_path(tmp_path, DEMO, config))
    with conn:
...
def test_rc6_audit_detects_clob_hash_tamper(tmp_path):
    demo_run(tmp_path, CONFIG)
    config = load_config(CONFIG)
    conn = connect(db_path(tmp_path, DEMO, config))
    with conn:
...
def test_rc6_audit_detects_proposed_final_and_value_tamper(tmp_path):
    demo_run(tmp_path, CONFIG)
    config = load_config(CONFIG)
    conn = connect(db_path(tmp_path, DEMO, config))
...
def test_rc6_negative_audit_framework_is_direct_and_detects_all(tmp_path):
    from src.forward_reporting_v5_1_7 import write_negative_tests_csv

    rows = write_negative_tests_csv(tmp_path, CONFIG)
    assert len(rows) == 30
    assert all(row["direct_data_modified"] == "true" for row in rows)
    assert all(row["synthetic_violation_event_inserted"] == "false" for row in rows)
    assert all(row["audit_command_executed"] == "true" for row in rows)
...
def test_v517_full_replay_detects_each_orderbook_fill_corruption(tmp_path, case, expected_error):
    from src.forward_reporting_v5_1_7 import apply_case
