...
import src.forward_simulation_v5_1_5 as sim  # noqa: E402

from src.forward_simulation_v5_1_5 import (  # noqa: E402
    DEMO,
    FixtureAdapter,
    STRATEGY_IDS,
    audit_integrity,
...
from src.polymarket_public_adapter_v5_1_5 import (  # noqa: E402
    AdapterError,
    PublicAdapter,
    calculate_fee,
    clob_token_pairs,
...
CONFIG = PROJECT_ROOT / "config/forward_simulation_v5_1_5.yaml"
...
def copy_v514_release_files(dst: Path) -> Path:
    for rel in [
        "src/forward_simulation_v5_1_5.py",
        "src/forward_reporting_v5_1_5.py",
        "src/polymarket_public_adapter_v5_1_5.py",
        "schemas/forward_simulation_v5_1_5.sql",
        "config/forward_simulation_v5_1_5.yaml",
    ]:
        target = dst / rel
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(PROJECT_ROOT / rel, target)
    for rel, body in {
        "reports/FORWARD_SIMULATION_V5_1_5_PREREGISTRATION.md": "preregistration\n",
        "reports/FORWARD_SIMULATION_V5_1_5_API_CONTRACT.md": "api contract\n",
        "reports/FORWARD_SIMULATION_V5_1_5_FEE_CONTRACT.md": "fee contract\n",
        "reports/FORWARD_SIMULATION_V5_1_5_SETTLEMENT_FINALITY_CONTRACT.md": "settlement finality contract\n",
    }.items():
        target = dst / rel
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(body, encoding="utf-8")
    return dst / "config/forward_simulation_v5_1_5.yaml"
...
def test_deleted_frozen_file_refuses_formal_write(tmp_path):
    ...
    (tmp_path / "reports/FORWARD_SIMULATION_V5_1_5_API_CONTRACT.md").unlink()
...
def test_resolved_raw_evidence_files_hash_recomputable(tmp_path):
    ...
    raw_dir = tmp_path / "data/forward_v5_1_5/demo/resolved_market_raw"
...
def test_rc4_infinite_loop_sleeps_between_three_rounds(monkeypatch, tmp_path):
    calls = []
    sleeps = []

    def fake_monitor(*args, **kwargs):
        calls.append(kwargs.get("run_id"))
        return {"run_id": kwargs.get("run_id"), "stopped": len(calls) >= 3}

    monkeypatch.setattr(sim, "monitor_once", fake_monitor)
...
def test_rc4_finite_loop_sleep_counts(monkeypatch, tmp_path):
    calls = []
    sleeps = []
    monkeypatch.setattr(sim, "monitor_once", lambda *a, **k: calls.append(1) or {"run_id": k.get("run_id")})
...
def test_rc4_single_iteration_does_not_sleep(monkeypatch, tmp_path):
    calls = []
    sleeps = []
    monkeypatch.setattr(sim, "monitor_once", lambda *a, **k: calls.append(1) or {"run_id": k.get("run_id")})
...
def test_rc4_pause_uses_low_frequency_wait_and_no_monitor(monkeypatch, tmp_path):
    init_ledger(tmp_path, DEMO, CONFIG)
...
def test_rc4_resume_after_pause_enters_normal_loop(monkeypatch, tmp_path):
    init_ledger(tmp_path, DEMO, CONFIG)
    monitor_control(tmp_path, DEMO, CONFIG, "pause")
    calls = []
    sleeps = []
...
def test_rc4_fatal_and_recoverable_errors(monkeypatch, tmp_path):
    sleeps = []
    monkeypatch.setattr(sim, "monitor_once", lambda *a, **k: {"fatal_error": True})
    ...
    monkeypatch.setattr(sim, "monitor_once", lambda *a, **k: sequence.pop(0))
    sleeps = []
...
def test_rc4_ctrl_c_releases_lock_and_marks_stopped(monkeypatch, tmp_path):
    sleeps = []
...
def test_rc4_monitor_once_uses_same_lock_and_rejects_second_instance(tmp_path):
    config = load_config(CONFIG)
    init_ledger(tmp_path, DEMO, CONFIG)
...
def test_rc4_formal_rejects_fixture_and_custom_adapter(tmp_path):
    init_ledger(tmp_path, "formal", CONFIG)
...
def test_rc4_gamma_real_tick_min_fields_are_read_and_crosschecked():
...
def test_rc4_only_gamma_constraints_do_not_allow_formal_orderbook():
...
def test_rc4_proposed_and_auto_resolved_proposed_never_final():
...
def test_rc4_winning_asset_is_strong_final_evidence():
...
def test_rc4_clob_public_token_winner_is_strong_final_evidence():
