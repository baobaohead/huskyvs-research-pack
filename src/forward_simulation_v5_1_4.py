#!/usr/bin/env python3
"""SQLite-backed weather forward simulation v5.1.4-RC3.

This module is standalone. It does not import v5, v5.1, v5.1.1, or v5.1.2.
Formal monitor commands use the v5.1.4 public adapter as the only live price
...
try:
    from src.polymarket_public_adapter_v5_1_4 import (
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
        AdapterError,
    ...
    from polymarket_public_adapter_v5_1_4 import (
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
        AdapterError,
...
VERSION = "forward_simulation_v5.1.4-rc3"
SCHEMA_VERSION = "forward_v5_1_4_schema_001"
FORMAL = "formal"
DEMO = "demo"
...
HASH_FILES = {
    "config_sha256": "config/forward_simulation_v5_1_4.yaml",
    "core_code_sha256": "src/forward_simulation_v5_1_4.py",
    "adapter_code_sha256": "src/polymarket_public_adapter_v5_1_4.py",
    "reporting_code_sha256": "src/forward_reporting_v5_1_4.py",
    "schema_sha256": "schemas/forward_simulation_v5_1_4.sql",
    "preregistration_sha256": "reports/FORWARD_SIMULATION_V5_1_4_PREREGISTRATION.md",
    "api_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_4_API_CONTRACT.md",
    "fee_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_4_FEE_CONTRACT.md",
}
...
def data_dir(root: Path, mode: str, config: dict[str, Any]) -> Path:
    if mode == LIVE:
        return root / str(config["paths"].get("live_integration_dir", "data/forward_v5_1_4/live_integration"))
    return root / str(config["paths"].get(f"{mode}_data_dir", f"data/forward_v5_1_4/{mode}"))
...
def rc3_dir(root: Path) -> Path:
    return root / "data/forward_v5_1_4/rc3"
...
def schema_path(root: Path, config: dict[str, Any]) -> Path:
    p = root / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_4.sql"))
    return p if p.exists() else PROJECT_ROOT / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_4.sql"))
...
@contextmanager
def acquire_monitor_lock(root: Path, mode: str, config: dict[str, Any], run_id: str, recover_stale_lock: bool = False, now: datetime | None = None):
    lock = lock_path(root, mode, config)
...
def frozen_file_records(root: Path, config_path: Path) -> dict[str, dict[str, Any]]:
    ...
    paths = {
        ...
        "core_code_sha256": root / "src/forward_simulation_v5_1_4.py",
        "adapter_code_sha256": root / "src/polymarket_public_adapter_v5_1_4.py",
        "reporting_code_sha256": root / "src/forward_reporting_v5_1_4.py",
        "schema_sha256": schema_path(root, config),
        "preregistration_sha256": root / str(config["paths"].get("preregistration_file", "reports/FORWARD_SIMULATION_V5_1_4_PREREGISTRATION.md")),
        "api_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_4_API_CONTRACT.md",
        "fee_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_4_FEE_CONTRACT.md",
    }
    ...
    try:
        ...
        with conn:
            ...
            for key, value in hashes.items():
                set_state(conn, key, value)
            append_audit(conn, FORMAL, "", "formal_started_v5_1_4", {"hashes": hashes}, "info", now)
...
def record_snapshot(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, purpose: str, raw_book: dict[str, Any], source_endpoint: str, now: datetime | None = None, gamma_market: dict[str, Any] | None = None) -> tuple[str, dict[str, Any], bool]:
...
def process_active_token_batch(
    ...
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, first_sig, "monitor", book_result.payload, book_result.url, now, first_gamma.payload)
    if not inserted:
...
def write_resolved_raw_evidence(root: Path, config: dict[str, Any], mode: str, run_id: str, sig: sqlite3.Row, gamma_result: Any, clob_result: Any, now: datetime | None = None) -> dict[str, Any]:
    pairs = clob_token_pairs(clob_result.payload) or gamma_token_pairs(gamma_result.payload)
    ...
    if mode == LIVE:
        targets.append(rc3_dir(root) / "resolved_market_raw" / name)
        ...
        with conn:
            ...
            for sig in sorted(signals.values(), key=lambda r: (r["created_at_utc"], r["signal_id"])):
                ...
                try:
                    ...
                    if state["market_status"] == "active_trading":
                        ...
                        write_resolved_raw_evidence(root, config, mode, rid, sig, gamma_result, clob_result, now)
                        results.extend(settle_signal_with_market(conn, rid, mode, sig, gamma_result.payload, clob_result.payload, gamma_result.url, now))
                    else:
            ...
            for token_id, bundle_rows in active_by_token.items():
                try:
                    ...
                    results.extend(process_active_token_batch(conn, rid, mode, token_id, bundle_rows, book_result, now))
                except Exception as exc:
...
def run_loop(
    ...
    interrupted = False
    with acquire_monitor_lock(root, mode, config, rid, recover_stale_lock=recover_stale_lock):
        try:
            while iterations <= 0 or completed < iterations:
                ...
                completed += 1
                write_monitor_heartbeat(root, mode, config, rid, "sleeping" if completed < iterations else "completed")
            ...
            try:
                ...
                conn.close()
            write_monitor_heartbeat(root, mode, config, rid, "interrupted")
...
def fetch_resolved_weather_evidence(root: Path, adapter: PublicAdapter, config: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    rc3_raw = rc3_dir(root) / "resolved_market_raw"
    ...
    for slug in config.get("live_integration", {}).get("resolved_weather_slugs", []):
        try:
            ...
            validations.append({"slug": slug, "settlement_status": evidence.get("settlement_status"), "evidence_valid": evidence.get("evidence_valid"), "token_settlement_values": evidence.get("token_settlement_values", {})})
            file_name = f"{str(slug).replace('/', '_')}_{content_hash(payload)[:12]}.json"
            write_json(out_dir / "resolved_market_raw" / file_name, payload)
            write_json(rc3_raw / file_name, payload)
            write_json(out_dir / "resolved_weather_evidence.json", payload)
            write_json(rc3_dir(root) / "resolved_market_validation.json", {"generated_at_utc": now_iso(), "validations": validations})
    ...
    write_json(rc3_dir(root) / "resolved_market_validation.json", result)
    return result
...
def live_integration(root: Path, config_path: Path, iterations: int, interval_seconds: Decimal, run_id: str | None = None) -> dict[str, Any]:
    ...
    write_json(rc3_dir(root) / "live_run_manifest.json", manifest)
    return manifest
...
def formal_empty_proof(root: Path, config_path: Path) -> dict[str, Any]:
    ...
    write_json(root / "data/forward_v5_1_4/formal_empty_proof.json", proof)
    write_json(rc3_dir(root) / "formal_empty_proof.json", proof)
    return proof
