...
def classify_exit(
    ...
    if not sells:
        return ["NO_RECORDED_SELL", "FINAL_PATH_UNKNOWN"]
    labels: list[str] = []
    ...
    if sold + EPSILON >= total_buy_shares:
        ...
        labels.append("PARTIAL_EXIT_REMAINDER_HELD")
    if any(
...
def attach_pnl(
    ...
    for event in events:
        ...
        open_pnl = sum(
            (finite(current_by_asset[a].get("cashPnl")) or 0)
        ...
        if strict_pnl is not None:
            status = "STRICT_CLOSED_SETTLED"
        elif current_assets:
            status = "CURRENT_OPEN_OR_UNREDEEMED"
        else:
            status = "OBSERVABLE_BUT_INCOMPLETE"
        out.append({
            **event,
            "pnl_status": status,
            ...
            "held_to_settlement_observed": bool(
            ...
            "current_open_mark_to_market_pnl": open_pnl if current_assets else None,
...
def archetype_labels(event: dict[str, Any], rows: list[dict[str, Any]]) -> list[str]:
    ...
    if "PARTIAL_EXIT_REMAINDER_HELD" in event["exit_classifications"]:
        labels.append("PARTIAL_PROFIT_EXIT" if (event.get("strict_pnl") or 0) >= 0 else "LOSS_CUT_EXIT")
    if event.get("held_to_settlement_observed"):
        labels.append("HOLD_TO_SETTLEMENT_OBSERVED")
    if not sells or event["pnl_status"] != "STRICT_CLOSED_SETTLED":
        labels.append("FINAL_PATH_UNKNOWN")
    ...
    for scope_name, scoped_events in scopes.items():
        for checkpoint in CHECKPOINTS:
            ...
            for event in scoped_events:
                ...
                if buys and buys[0]["timestamp_epoch"] <= cutoff:
                    first_before += 1
                elif buys:
                    first_after += 1
                if any(row["timestamp_epoch"] > cutoff for row in buys):
                    continued += 1
                for fraction in THRESHOLDS:
...
def timing_comparison(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for label, predicate in (
        ("profit_events", lambda event: float(event["strict_pnl"]) > 0),
        ("loss_events", lambda event: float(event["strict_pnl"]) < 0),
    ):
        subset = [event for event in events if event.get("strict_pnl") is not None and predicate(event)]
        result[label] = {
            ...
            "median_initial_buy_share": statistics.median(
                [event["initial_buy_share_of_final_buy_usd"] for event in subset]
            ) if subset else None,
            "median_total_buy_usd": statistics.median([event["total_buy_usd"] for event in subset]) if subset else None,
            "median_buy_duration_hours": statistics.median(
                [event["buy_duration_seconds"] / 3600 for event in subset if event["buy_duration_seconds"] is not None]
            ) if subset else None,
            "median_d_minus_1_buy_usd_share": statistics.median(
                [event["d_minus_1_buy_usd_share"] for event in subset if event["d_minus_1_buy_usd_share"] is not None]
            ) if subset else None,
            "median_d0_buy_usd_share": statistics.median(
                [event["d0_buy_usd_share"] for event in subset if event["d0_buy_usd_share"] is not None]
            ) if subset else None,
            "median_d0_after_1200_buy_usd_share": statistics.median(
                [event["d0_after_1200_buy_usd_share"] for event in subset if event["d0_after_1200_buy_usd_share"] is not None]
            ) if subset else None,
            "median_d0_after_1400_buy_usd_share": statistics.median(
                [event["d0_after_1400_buy_usd_share"] for event in subset if event["d0_after_1400_buy_usd_share"] is not None]
            ) if subset else None,
            "median_d0_after_1500_buy_usd_share": statistics.median(
                [event["d0_after_1500_buy_usd_share"] for event in subset if event["d0_after_1500_buy_usd_share"] is not None]
            ) if subset else None,
            "median_bucket_count": statistics.median([event["outcome_token_count"] for event in subset]) if subset else None,
            "adjacent_basket_event_share": (
                sum(bool(event["adjacent_bucket_pairs"]) for event in subset) / len(subset)
                if subset else None
            ),
            "price_up_add_count": sum(
                1 for event in subset for _ in range(event.get("price_up_add_count", 0))
            ),
            "price_down_add_count": sum(
                1 for event in subset for _ in range(event.get("price_down_add_count", 0))
            ),
            "median_first_sell_time": median_relative_time(subset, "first_sell_time_cst"),
            "median_recorded_sell_share_fraction": statistics.median(
                [event["recorded_sell_share_fraction"] for event in subset if event["recorded_sell_share_fraction"] is not None]
            ) if subset else None,
            "median_holding_to_first_sell_hours": statistics.median(
                [event["holding_duration_to_first_sell_seconds"] / 3600 for event in subset if event["holding_duration_to_first_sell_seconds"] is not None]
            ) if subset else None,
        }
    return result
...
def summarize(
    ...
    return {
        ...
        "earliest_history_confidence": "HIGH",
        "latest_history_confidence": "HIGH" if cutoff_epoch - last_ts < 14 * 86400 else "MEDIUM",
        ...
        "history_gap_result": {
            ...
            "note": "无成交月份不等于接口缺口；请求清单记录了每个成功空窗。",
        },
        ...
        "events_held_to_settlement_observed": sum(event["held_to_settlement_observed"] for event in events),
        ...
        "observable_but_incomplete_event_count": sum(event["pnl_status"] == "OBSERVABLE_BUT_INCOMPLETE" for event in events),
        "observable_but_incomplete_pnl": sum(
            event.get("observable_but_incomplete_pnl") or 0 for event in events
        ),
        "current_open_event_count": sum(event["pnl_status"] == "CURRENT_OPEN_OR_UNREDEEMED" for event in events),
        "current_open_mark_to_market_pnl": sum(
            event.get("current_open_mark_to_market_pnl") or 0 for event in events
        ),
        "current_open_snapshot_at_utc": manifest.get("generated_at_utc"),
        ...
        "beijing_total_pnl_with_open_mark_to_market": total_strict_pnl + sum(
            event.get("current_open_mark_to_market_pnl") or 0 for event in events
        ),
...
def render_report(summary: dict[str, Any], events: list[dict[str, Any]]) -> str:
    ...
    dominant_exit = sorted(
        (
            (label, archetype_counts.get(label, 0))
            for label in (
                "QUICK_FLIP", "PARTIAL_PROFIT_EXIT", "LOSS_CUT_EXIT",
                "HOLD_TO_SETTLEMENT_OBSERVED", "FINAL_PATH_UNKNOWN",
            )
        ),
        key=lambda item: (-item[1], item[0]),
    )[:3]
    ...
    return "\n".join([
        ...
        f"16. PARTIAL_PROFIT_EXIT / LOSS_CUT_EXIT 研究标签事件为 {archetype_counts.get('PARTIAL_PROFIT_EXIT', 0)} / {archetype_counts.get('LOSS_CUT_EXIT', 0)}。",
...
def analyze(
    ...
    *,
    ...
    events = attach_pnl(events, fills_by_event, repo_root, current, closed)
    for event in events:
