#!/usr/bin/env python3
"""SQLite-backed weather forward simulation v5.1.7-RC6.

This module is standalone. It does not import v5, v5.1, v5.1.1, or v5.1.2.
Formal monitor commands use the v5.1.7 public adapter as the only live price
...
HASH_FILES = {
    "config_sha256": "config/forward_simulation_v5_1_7.yaml",
    "core_code_sha256": "src/forward_simulation_v5_1_7.py",
    "adapter_code_sha256": "src/polymarket_public_adapter_v5_1_7.py",
    "reporting_code_sha256": "src/forward_reporting_v5_1_7.py",
    "schema_sha256": "schemas/forward_simulation_v5_1_7.sql",
    "preregistration_sha256": "reports/FORWARD_SIMULATION_V5_1_7_PREREGISTRATION.md",
    "api_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_7_API_CONTRACT.md",
    "fee_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_7_FEE_CONTRACT.md",
    "settlement_finality_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_7_SETTLEMENT_FINALITY_CONTRACT.md",
}
...
def data_dir(root: Path, mode: str, config: dict[str, Any]) -> Path:
    if mode == LIVE:
        return root / str(config["paths"].get("live_integration_dir", "data/forward_v5_1_7/live_integration"))
    return root / str(config["paths"].get(f"{mode}_data_dir", f"data/forward_v5_1_7/{mode}"))
...
def rc6_dir(root: Path) -> Path:
    return root / "data/forward_v5_1_7/rc6"
...
def schema_path(root: Path, config: dict[str, Any]) -> Path:
    p = root / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_7.sql"))
    return p if p.exists() else PROJECT_ROOT / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_7.sql"))
...
def frozen_file_records(root: Path, config_path: Path) -> dict[str, dict[str, Any]]:
    ...
    paths = {
        ...
        "core_code_sha256": root / "src/forward_simulation_v5_1_7.py",
        "adapter_code_sha256": root / "src/polymarket_public_adapter_v5_1_7.py",
        "reporting_code_sha256": root / "src/forward_reporting_v5_1_7.py",
        "schema_sha256": schema_path(root, config),
        "preregistration_sha256": root / str(config["paths"].get("preregistration_file", "reports/FORWARD_SIMULATION_V5_1_7_PREREGISTRATION.md")),
        "api_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_7_API_CONTRACT.md",
        "fee_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_7_FEE_CONTRACT.md",
        "settlement_finality_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_7_SETTLEMENT_FINALITY_CONTRACT.md",
    }
    ...
    try:
        ...
        with conn:
            ...
            for key, value in hashes.items():
                set_state(conn, key, value)
            append_audit(conn, FORMAL, "", "formal_started_v5_1_7", {"hashes": hashes}, "info", now)
...
def record_snapshot(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, purpose: str, raw_book: dict[str, Any], source_endpoint: str, now: datetime | None = None, gamma_market: dict[str, Any] | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None, lock_id: str = "") -> tuple[str, dict[str, Any], bool]:
    book = normalize_orderbook(raw_book, sig["token_id"], sig["condition_id"], gamma_market)
    config_path = config_path or (root / "config/forward_simulation_v5_1_7.yaml")
    ...
    conn.execute(
        """
        INSERT INTO orderbook_snapshots(run_id,snapshot_id,lock_id,content_hash,captured_at_utc,token_id,condition_id,market_slug,purpose,best_bid,best_ask,spread,tick_size,min_order_size,neg_risk,raw_orderbook_json,raw_response,raw_response_sha256,response_content_type,http_status_code,request_params_json,server_time_utc,normalized_bids_json,normalized_asks_json,normalized_book_json,normalized_book_sha256,normalization_algorithm_version,market_constraints_hash,bid_levels_count,ask_levels_count,total_bid_shares,total_ask_shares,source_endpoint,adapter_name,adapter_version,adapter_code_hash,data_source,run_environment,raw_response_hash,gamma_tick_size,gamma_min_order_size,clob_tick_size,clob_min_order_size,selected_tick_size,selected_min_order_size,constraint_crosscheck_status,constraint_conflict_details,mode)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            run_id,
...
def snapshot_meta(conn: sqlite3.Connection, snapshot_id: str, mode: str) -> dict[str, str]:
    row = conn.execute("SELECT adapter_name,adapter_version,adapter_code_hash,data_source,run_environment,raw_response_hash,raw_response_sha256,normalized_book_sha256,tick_size,min_order_size FROM orderbook_snapshots WHERE snapshot_id=? AND mode=? ORDER BY row_id DESC LIMIT 1", (snapshot_id, mode)).fetchone()
    if not row:
        return {"adapter_name": "", "adapter_version": "", "adapter_code_hash": "", "data_source": "", "run_environment": mode, "raw_response_hash": "", "raw_response_sha256": "", "normalized_book_sha256": "", "tick_size": "", "min_order_size": ""}
...
def process_entry_with_bundle(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, bundle: MarketBundle, now: datetime | None = None, lock_id: str = "") -> dict[str, Any]:
    ...
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, sig, "entry", bundle.raw_book, bundle.book_result.url, now, bundle.gamma_market, lock_id=lock_id)
...
def process_exit_with_bundle(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, bundle: MarketBundle, now: datetime | None = None, lock_id: str = "") -> list[dict[str, Any]]:
    ...
    if fee_policy["fee_crosscheck_status"] in {"conflict", "unknown"}:
        ...
        return [{"signal_id": sig["signal_id"], "status": "fee_policy_rejected"}]
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, sig, "exit", bundle.raw_book, bundle.book_result.url, now, bundle.gamma_market, lock_id=lock_id)
    validation = validate_token_mapping(dict(sig), bundle.gamma_market, bundle.clob_info, book)
    if not validation["mapping_valid"]:
...
def process_active_token_batch(
    ...
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, first_sig, "monitor", book_result.payload, book_result.url, now, first_gamma.payload, root, config_path, lock_id)
    if not inserted:
...
def settle_signal_with_market(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, gamma_market: dict[str, Any], clob_info: dict[str, Any], source_endpoint: str, now: datetime | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None, clob_public_info: dict[str, Any] | None = None, lock_id: str = "") -> list[dict[str, Any]]:
    ...
    for strategy_id in STRATEGY_IDS:
        ...
        meta = source_meta(root, config_path or (root / "config/forward_simulation_v5_1_7.yaml"), mode, source_endpoint, evidence["raw_response_hash"])
...
def write_resolved_raw_evidence(root: Path, config: dict[str, Any], mode: str, run_id: str, sig: sqlite3.Row, gamma_result: Any, clob_result: Any, now: datetime | None = None, clob_public_result: Any | None = None) -> dict[str, Any]:
    ...
    if mode == LIVE:
        targets.append(rc6_dir(root) / "resolved_market_raw" / name)
...
def audit_orderbook_snapshot_replay(conn: sqlite3.Connection, mode: str, checks: dict[str, Any]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for snap in conn.execute("SELECT * FROM orderbook_snapshots WHERE mode=?", (mode,)):
        ...
        try:
            ...
            continue
        try:
            book = normalize_orderbook(raw_book, snap["token_id"], snap["condition_id"])
...
def audit_fill_replay(conn: sqlite3.Connection, mode: str, checks: dict[str, Any], books: dict[tuple[str, str], dict[str, Any]]) -> None:
    ...
    for snap_key, book in books.items():
        ...
        for row in stable_entries:
            ...
            policy = fee_policy_from_fill_row(row)
            ...
            for sig in sorted(signals.values(), key=lambda r: (r["created_at_utc"], r["signal_id"])):
                ...
                try:
                    ...
                    if state["market_status"] == "active_trading":
                        ...
                        results.extend(settle_signal_with_market(conn, rid, mode, sig, gamma_result.payload, clob_result.payload, gamma_result.url, now, root, config_path, clob_public_result.payload, lock_id=lock_id))
                    else:
...
def audit_integrity(root: Path, mode: str, config_path: Path, level: str = "full-replay") -> dict[str, Any]:
    ...
    try:
        ...
        return {"ok": ok, "level": level, "checks": checks}
    finally:
        conn.close()
...
def fetch_resolved_weather_evidence(root: Path, adapter: PublicAdapter, config: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    rc6_raw = rc6_dir(root) / "resolved_market_raw"
    ...
    for slug in config.get("live_integration", {}).get("resolved_weather_slugs", []):
        try:
            ...
            write_json(rc6_raw / file_name, payload)
            write_json(out_dir / "resolved_weather_evidence.json", payload)
            write_json(rc6_dir(root) / "resolved_market_validation.json", {"generated_at_utc": now_iso(), "validations": validations})
    ...
    write_json(rc6_dir(root) / "resolved_market_validation.json", result)
    return result
...
def live_integration(root: Path, config_path: Path, iterations: int, interval_seconds: Decimal, run_id: str | None = None) -> dict[str, Any]:
    ...
    write_json(rc6_dir(root) / "live_run_manifest.json", manifest)
    write_json(rc6_dir(root) / "real_signal_to_fill_validation.json", signal_to_fill)
    return manifest
...
def formal_empty_proof(root: Path, config_path: Path) -> dict[str, Any]:
    ...
    write_json(root / "data/forward_v5_1_7/formal_empty_proof.json", proof)
    write_json(rc6_dir(root) / "formal_empty_proof.json", proof)
    return proof
