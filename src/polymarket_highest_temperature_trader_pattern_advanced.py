#!/usr/bin/env python3
"""Reusable advanced analysis for highest-temperature trader patterns.

This module consumes normalized public fills or the existing first-stage
``all_fills.csv``/``_public_evidence`` files. It never calls a network endpoint.
The registered price buckets come from the basic Skill implementation.
"""
...
from .polymarket_highest_temperature_trader_pattern_v1 import price_band
PRICE_BANDS = (
    "PRICE_0_10C",
    "PRICE_10_30C",
    "PRICE_30_70C",
...
STYLE_MARKET_LIKE_MIN_SAME_HOUR_TWO_WAY = 20
STYLE_MARKET_LIKE_MIN_SHORT_HOLD_RATIO = 0.50
STYLE_MARKET_LIKE_MIN_SELL_REBUY_RATIO = 0.30
...
def event_records_from_payloads(
...
def event_records(root: Path, wallet: str) -> list[dict[str, Any]]:
    evidence_root = root / "_public_evidence" / wallet
    return event_records_from_payloads(
        read_json(evidence_root / "event_audit.json"),
        read_json(evidence_root / "target_markets.json"),
    )


def market_weather_day_universe(
    start: date,
    end: date,
    events: Iterable[dict[str, Any]],
    requested_cities: Iterable[str] | None = None,
) -> list[tuple[str, date]]:
    """Return the real highest-temperature market days in scope.

    The event/audit evidence is the authority for whether a city/date exists.
    Wallet fills are deliberately not consulted, and duplicate event slugs
    for one city/date collapse to one market weather day.
    """
    requested = {str(city) for city in requested_cities or ()}
    keys: set[tuple[str, date]] = set()
    for event in events:
        city = str(event.get("canonical_city") or "")
        slug = str(event.get("event_slug") or "")
        raw_date = event.get("weather_date")
        if not city or not slug or not raw_date:
            continue
        if requested and city not in requested:
            continue
        try:
            weather_date = parse_date(str(raw_date))
        except ValueError:
            continue
        if start <= weather_date <= end:
            keys.add((city, weather_date))
    return sorted(keys)
...
def date_denominator(
    ...
    requested_city_set = {str(city) for city in requested_cities or ()}
    in_range_all = [event for event in events if start <= parse_date(event["weather_date"]) <= end]
    in_range = [
        event for event in in_range_all
        if not requested_city_set or str(event.get("canonical_city") or "") in requested_city_set
    ]
    out_of_range = [event for event in events if event not in in_range_all]
    ...
    market_weather_day_keys = market_weather_day_universe(start, end, in_range, None)
    market_cities = sorted({city for city, _ in market_weather_day_keys})
    city_names = tuple(requested_cities) if requested_city_set else tuple(market_cities)
    city_weather_day_keys = set(market_weather_day_keys)
    ...
    return {
        ...
        "market_city_count": len(market_cities),
        ...
        "market_weather_day_count": len(city_weather_day_keys),
        "market_weather_day_keys": [[city, day.isoformat()] for city, day in market_weather_day_keys],
...
def temperature_day_summary(
    ...
    market_weather_day_keys: Iterable[tuple[str, date]],
) -> list[dict[str, Any]]:
    by_date = group_by_date(rows)
    result = []
    for city, day in sorted(set(market_weather_day_keys)):
...
def high_sell_analysis(paths: list[dict[str, Any]]) -> dict[str, Any]:
    high_paths = []
    for path in paths:
...
# Compatibility name for the reviewed second-stage tests and old research
# notebooks. The implementation is wallet-agnostic despite this legacy name.
wallet_two_high_sell_analysis = high_sell_analysis
...
def market_maker_like_activity(metrics: dict[str, Any]) -> bool:
    """Flag high-frequency two-way behavior without assigning a maker label."""
    return (
        metrics["same_hour_two_way"] >= STYLE_MARKET_LIKE_MIN_SAME_HOUR_TWO_WAY
        and metrics["short_hold_ratio_decimal"] >= STYLE_MARKET_LIKE_MIN_SHORT_HOLD_RATIO
        and metrics["sell_then_rebuy_ratio_decimal"] >= STYLE_MARKET_LIKE_MIN_SELL_REBUY_RATIO
    )
...
def wallet_style(
    ...
    market_days = sorted(set(market_weather_day_keys))
    top_days = []
    for city, day in market_days:
    ...
    metrics = {
        ...
        "average_fills_per_calendar_day": len(rows) / len(requested_dates) if requested_dates else None,
        "average_fills_per_market_weather_day": len(rows) / len(market_days) if market_days else None,
        # Compatibility alias retained for consumers of the previous advanced JSON.
    ...
    metrics["market_maker_like_activity"] = market_maker_like_activity(metrics)
...
def build_machine_outputs(
    ...
    *,
    json_filename: str = "SECOND_STAGE_TRADER_PATTERN_COMPARISON.json",
) -> None:
    ...
    json_payload = {
        ...
        "thresholds": {
            ...
            "style": {
                ...
                "market_maker_like_min_same_hour_two_way": STYLE_MARKET_LIKE_MIN_SAME_HOUR_TWO_WAY,
                "market_maker_like_min_short_hold_ratio": STYLE_MARKET_LIKE_MIN_SHORT_HOLD_RATIO,
                "market_maker_like_min_sell_rebuy_ratio": STYLE_MARKET_LIKE_MIN_SELL_REBUY_RATIO,
            },
        },
        "events": events,
        "wallets": {
...
def analyze_wallet_rows(
    raw_rows: list[dict[str, Any]],
    events: list[dict[str, Any]],
    requested_dates: list[date],
    quality: dict[str, Any] | None = None,
    requested_cities: Iterable[str] | None = None,
    market_weather_day_keys: Iterable[tuple[str, date]] | None = None,
) -> dict[str, Any]:
    rows = normalize_fills(raw_rows)
    paths = asset_paths(rows)
    if market_weather_day_keys is None and requested_dates:
        market_weather_day_keys = market_weather_day_universe(
            min(requested_dates), max(requested_dates), events, requested_cities,
        )
    market_weather_day_keys = tuple(market_weather_day_keys or ())
    daily_temperature = temperature_day_summary(rows, market_weather_day_keys)
    wallet_data = {
        "rows": rows,
        "quality": quality or {},
        "paths": paths,
        ...
        "style": wallet_style(paths, rows, requested_dates, market_weather_day_keys),
    }
    for side, outcome in (("BUY", "YES"), ("BUY", "NO"), ("SELL", "YES"), ("SELL", "NO")):
        scoped = [row for row in rows if row["_side"] == side and row["_outcome"] == outcome]
        key = f"{side} {outcome}"
        wallet_data["time"][key] = side_outcome_time_table(scoped)
        wallet_data["price"][key] = price_table(scoped)
    return wallet_data
...
def make_report(
    ...
    events = event_records(root, wallets[0])
    requested_cities = [city] if city != "all-cities" else []
    denominator = date_denominator(root, start, end, events, requested_cities)
    market_weather_day_keys = [
        (city_name, parse_date(weather_date))
        for city_name, weather_date in denominator["market_weather_day_keys"]
    ]
    wallet_data = {}
    for wallet in wallets:
        quality = quality_summary(root, wallet)
        wallet_data[wallet] = analyze_wallet_rows(
            read_csv(root / wallet / "all_fills.csv"), events, requested_dates, quality,
            requested_cities, market_weather_day_keys,
        )
    ...
    report_path.write_text(
        render_report(root, report_path, start, end, city, requested_dates, events, denominator, wallet_data, comparison_high, wallets).replace("\n+  --", "\n  --"),
        encoding="utf-8",
    )
    build_machine_outputs(report_path.parent, start, end, city, requested_dates, events, denominator, wallet_data, high_analyses, wallets)
...
def render_style(style: dict[str, Any]) -> str:
    return md_table(
        ...
        [
            ...
            ["每个自然日平均成交笔数", fmt_num(style["average_fills_per_calendar_day"])],
            ["每个真实市场天气日平均成交笔数", fmt_num(style["average_fills_per_market_weather_day"])],
            ...
            ["MARKET_MAKER_LIKE_ACTIVITY", str(style["market_maker_like_activity"]).lower()],
        ],
    )
...
def render_report(
    ...
    w1 = data[wallets[0]]
    w2 = data[wallets[1]] if len(wallets) > 1 else w1
    w1_rows = w1["rows"]
    w2_rows = w2["rows"]
    ...
    lines = [
        ...
        f"MARKET_WEATHER_DAY_COUNT={denominator.get('market_weather_day_count', denominator.get('city_weather_day_count', denominator['unique_weather_dates']))}",
    ...
    def style_reason(style: dict[str, Any]) -> str:
        ...
        if style["market_maker_like_activity"]:
            return f"存在类似高频双向交易的行为特征（同小时双向组数{style['same_hour_two_way']}），但没有maker/taker证据，不能判断为做市商。"
        return "当前指标没有满足预设的主动再平衡或买入主导阈值。"
    ...
    lines.extend(["", "## 8. 温度组合：互斥主分类", "", f"每个{calendar_day_count}个真实市场天气日严格分入一个类别；没有真实市场的城市/日期不生成NO_BUY。重复天气日期的多个 event 在天气日级合并，重复 event 不重复计日。yes_bucket_count/no_bucket_count 是该天气日 BUY 记录中的唯一温度桶数。"])
    for wallet in wallets:
    ...
    lines.extend(render_dynamic_conclusion_sections(root, report_path, city, requested_dates, data, w2_high, wallets))
    return "\n".join(lines) + "\n"
...
def render_wallet_summary(
    ...
    high_analysis: dict[str, Any],
    advanced_status: str = "READY",
) -> str:
    """Render the Chinese, plain-language advanced report for one wallet."""
    city_names = sorted({day.get("canonical_city", "") for day in wallet_data.get("temperature_days", []) if day.get("canonical_city", "")})
    ...
    if advanced_status != "READY":
        return "\n".join([
            ...
            f"DUPLICATE_EVENT_DATE_COUNT={denominator['duplicate_event_date_count']}",
            "",
            "证据未达到 READY，已暂停高级模式结论；请先修复公开证据完整性。",
            "不输出建仓时间、价格偏好、温度组合、风格或低买高卖策略判断。",
        ]) + "\n"
    style = wallet_data["style"]
    low = high_analysis["low_buy_high_sell"]
    partial = low["all_partial_asset_count"]
    near = low["near_full_asset_count"]
    mixed = low["mixed_asset_count"]
    total_exit_assets = partial + near + mixed + low["exceeds_asset_count"] + low["unknown_asset_count"]
    if near > partial:
        exit_sentence = "接近全部观察库存退出的资产更多"
    elif partial > near:
    ...
    lines = [
        ...
        *( [""] if empty_category_notes else [] ),
        ...
        f"- 高价SELL退出：{exit_sentence}；资产级为ALL_PARTIAL={partial}/{total_exit_assets}、HAS_NEAR_FULL={near}/{total_exit_assets}、MIXED={mixed}/{total_exit_assets}。",
        f"- SELL后重新BUY：{style['sell_then_rebuy_ratio']:.2f}%的有BUY/SELL资产组；同小时双向组数{style['same_hour_two_way']}。",
        f"- MARKET_MAKER_LIKE_ACTIVITY={str(style['market_maker_like_activity']).lower()}；没有maker/taker时不输出做市商标签。",
        "",
        "## 证据与明细",
        "",
        "### BUY / SELL四类时间分布",
    ...
    for key in ("BUY YES", "BUY NO", "SELL YES", "SELL NO"):
        lines.extend([f"#### {key}", "", render_time_table(wallet_data["time"][key]), ""])
    lines.extend(["### 每个天气日的时间路径", "", details(f"{len(wallet_data['event_paths'])}个event记录", render_event_path_table(wallet_data["event_paths"])), "", "### 完整价格带", ""])
    for key in ("BUY YES", "BUY NO", "SELL YES", "SELL NO"):
        lines.extend([f"#### {key}", "", render_price_table(wallet_data["price"][key]), ""])
    lines.extend([
        "### 逐资产路径",
        "",
        ...
        "### 高价SELL与逐笔观察库存",
        "",
        render_high_cases(high_analysis),
        "",
        "### 风格指标",
        "",
        render_style(style),
        "",
        "中文判断：存在类似高频双向交易的行为特征，但没有maker/taker证据，不能判断为做市商。" if style["market_maker_like_activity"] else "中文判断：没有达到高频双向行为旗标。",
        "",
        "### 互斥温度组合",
        "",
        ...
        "",
        md_table(["指标", "结果"], [
            ["YES档数P25/P50/P75/P90", "/".join("—" if value is None else f"{value:.2f}" for value in wallet_data["multi_yes"]["yes_bucket_count_percentiles"].values())],
            ["主力YES档USD占比P25/P50/P75/P90", "/".join("—" if value is None else f"{value:.2f}%" for value in wallet_data["multi_yes"]["main_usd_share_percentiles"].values())],
            ["主力档最贵/最便宜", f"{wallet_data['multi_yes']['main_most_expensive_count']}/{wallet_data['multi_yes']['yes_active_days']}；{wallet_data['multi_yes']['main_cheapest_count']}/{wallet_data['multi_yes']['yes_active_days']}"],
            ["便宜尾部YES USD/shares占比", f"{fmt_pct(wallet_data['multi_yes']['cheap_tail_usd_share'])} / {fmt_pct(wallet_data['multi_yes']['cheap_tail_shares_share'])}"],
            ["相邻档价格差P25/P50/P75/P90", "/".join("—" if value is None else f"{value:.2f}¢" for value in wallet_data["multi_yes"]["adjacent_price_gap_percentiles_cents"].values())],
        ]),
        "",
        "### NO在组合中的位置",
        "",
        md_table(["指标", "结果"], [
            ["NO位置", "; ".join(f"{key}:{value}日" for key, value in sorted(wallet_data["no"]["position_counts"].items())) or "无"],
            ["YES先买/NO先买", f"{wallet_data['no']['first_buy_counts'].get('YES先买', 0)} / {wallet_data['no']['first_buy_counts'].get('NO先买', 0)}"],
            ["NO晚于YES", wallet_data["no"]["late_counts"].get("NO晚于YES", 0)],
            ["NO投入USD占比P25/P50/P75/P90", "/".join("—" if value is None else f"{value:.2f}%" for value in wallet_data["no"]["daily_no_usd_share_percentiles"].values())],
        ]),
        "",
        "## 可以确认与不能确认",
        "",
        f"可以确认：公开fills中有{low['assets']}个同资产低价BUY→高价SELL路径，风格指标支持{style['style_label']}，温度组合和NO位置均按天气日统计。",
        "不能确认：完整账户库存、未成交订单、撤单、主观预测、盈利、PnL/ROI/胜率，以及高价SELL是否来自公开记录之外的库存。",
        "",
        "本报告不把低价BUY→高价SELL称为盈利交易，只称为公开成交中观察到的同资产价格路径。",
    ])
    return "\n".join(lines) + "\n"


def advanced_comparison_payload(
    start: date,
    end: date,
    city: str,
    requested_dates: list[date],
    ...
    wallets = tuple(data)
    cities = list(denominator.get("market_cities", []))
    return jsonable({
        ...
        "scope": {
            ...
            "requested_calendar_day_count": len(requested_dates),
            "city_weather_day_count": denominator.get("city_weather_day_count", denominator.get("unique_weather_dates", 0)),
            "market_weather_day_count": denominator.get("market_weather_day_count", denominator.get("city_weather_day_count", 0)),
        },
        "denominator": denominator,
        "wallets": {
            wallet: {
                "advanced_status": data[wallet].get("quality", {}).get("pattern_report_status", "READY"),
                "style": data[wallet]["style"],
                "path_metrics": data[wallet]["path_metrics"],
                "category_counts": dict(data[wallet]["category_counts"]),
                "multi_yes": data[wallet]["multi_yes"],
...
def render_advanced_comparison(
    ...
    lines = [
        ...
        "",
        "各钱包详细中文报告见各自目录的 `advanced_summary.md`。",
    ]
    return "\n".join(lines) + "\n"
...
def run_advanced_analysis(
    wallets: Iterable[str],
    raw_rows_by_wallet: dict[str, list[dict[str, Any]]],
    events: list[dict[str, Any]],
    output_root: Path,
    start: date,
    end: date,
    ...
    wallets = tuple(wallets)
    requested_dates = date_range(start, end)
    requested_city_scope = tuple(requested_cities or ())
    denominator = date_denominator(output_root, start, end, events, requested_city_scope)
    market_weather_day_keys = [
        (city_name, parse_date(weather_date))
        for city_name, weather_date in denominator["market_weather_day_keys"]
    ]
    ...
    data = {
        wallet: analyze_wallet_rows(
            raw_rows_by_wallet[wallet], events, requested_dates,
            quality_by_wallet.get(wallet, {}), requested_city_scope,
            market_weather_day_keys,
        )
        for wallet in wallets
    }
    ...
    for wallet in wallets:
        wallet_dir = output_root / wallet
        wallet_dir.mkdir(parents=True, exist_ok=True)
        build_machine_outputs(
            wallet_dir, start, end, city, requested_dates, events, denominator,
            {wallet: data[wallet]}, {wallet: high_analyses[wallet]}, (wallet,),
            json_filename="advanced_summary.json",
        )
        status = str(quality_by_wallet.get(wallet, {}).get("pattern_report_status", "READY"))
        (wallet_dir / "advanced_summary.md").write_text(
            render_wallet_summary(wallet, start, end, city, requested_dates, denominator, data[wallet], high_analyses[wallet], status),
            encoding="utf-8",
        )
    (output_root / "advanced_trader_comparison.json").write_text(
        json.dumps(advanced_comparison_payload(start, end, city, requested_dates, denominator, data, high_analyses), ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    (output_root / "advanced_trader_comparison.md").write_text(
        render_advanced_comparison(start, end, city, requested_dates, denominator, data, high_analyses),
        encoding="utf-8",
    )
    return {"denominator": denominator, "wallet_data": data, "high_analyses": high_analyses}
