#!/usr/bin/env python3
"""Public read-only Polymarket adapter for v5.1.4-RC3.
...
ADAPTER_VERSION = "polymarket_public_adapter_v5.1.4-rc3"
GAMMA_BASE = "https://gamma-api.polymarket.com"
CLOB_BASE = "https://clob.polymarket.com"
USER_AGENT = "huskyvs-v5.1.4-rc3-readonly/1.0"
ZERO = Decimal("0")
ONE = Decimal("1")
...
def market_accepting_orders(market: dict[str, Any]) -> bool:
    ...
    if accepting is None:
        accepting = market.get("enableOrderBook")
    return accepting is not False


def market_state(market: dict[str, Any]) -> dict[str, Any]:
    ...
    accepting = market_accepting_orders(market)
    if disputed:
        status = "disputed"
    elif resolved:
        status = "resolved"
...
def normalize_orderbook(raw: dict[str, Any], expected_token_id: str | None = None, expected_condition_id: str | None = None, gamma_market: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if raw.get("tick_size") in ("", None) and raw.get("tickSize") in ("", None):
        raise AdapterError("constraints_unknown", "orderbook tick_size is missing")
    if raw.get("min_order_size") in ("", None) and raw.get("minOrderSize") in ("", None):
        raise AdapterError("constraints_unknown", "orderbook min_order_size is missing")
    tick_size = dec(raw.get("tick_size") if raw.get("tick_size") not in ("", None) else raw.get("tickSize"))
    min_order_size = dec(raw.get("min_order_size") if raw.get("min_order_size") not in ("", None) else raw.get("minOrderSize"))
    if tick_size <= ZERO or min_order_size <= ZERO:
        raise AdapterError("invalid_constraints", "tick_size and min_order_size must be positive")
    if gamma_market:
        gamma_tick = gamma_market.get("tick_size") or gamma_market.get("tickSize") or gamma_market.get("minimumTickSize")
        gamma_min = gamma_market.get("min_order_size") or gamma_market.get("minOrderSize") or gamma_market.get("minimumOrderSize")
        if gamma_tick not in ("", None) and dec(gamma_tick) != tick_size:
            raise AdapterError("constraints_conflict", "gamma tick_size conflicts with orderbook tick_size")
        if gamma_min not in ("", None) and dec(gamma_min) != min_order_size:
            raise AdapterError("constraints_conflict", "gamma min_order_size conflicts with orderbook min_order_size")
