#!/usr/bin/env python3
"""SQLite-backed weather forward simulation v5.1.5-RC4.

This module is standalone. It does not import v5, v5.1, v5.1.1, or v5.1.2.
Formal monitor commands use the v5.1.5 public adapter as the only live price
...
try:
    from src.polymarket_public_adapter_v5_1_5 import (
        ADAPTER_NAME,
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
    ...
    from polymarket_public_adapter_v5_1_5 import (
        ADAPTER_NAME,
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
...
VERSION = "forward_simulation_v5.1.5-rc4"
SCHEMA_VERSION = "forward_v5_1_5_schema_001"
FORMAL = "formal"
DEMO = "demo"
...
HASH_FILES = {
    "config_sha256": "config/forward_simulation_v5_1_5.yaml",
    "core_code_sha256": "src/forward_simulation_v5_1_5.py",
    "adapter_code_sha256": "src/polymarket_public_adapter_v5_1_5.py",
    "reporting_code_sha256": "src/forward_reporting_v5_1_5.py",
    "schema_sha256": "schemas/forward_simulation_v5_1_5.sql",
    "preregistration_sha256": "reports/FORWARD_SIMULATION_V5_1_5_PREREGISTRATION.md",
    "api_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_5_API_CONTRACT.md",
    "fee_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_5_FEE_CONTRACT.md",
    "settlement_finality_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_5_SETTLEMENT_FINALITY_CONTRACT.md",
}
...
def data_dir(root: Path, mode: str, config: dict[str, Any]) -> Path:
    if mode == LIVE:
        return root / str(config["paths"].get("live_integration_dir", "data/forward_v5_1_5/live_integration"))
    return root / str(config["paths"].get(f"{mode}_data_dir", f"data/forward_v5_1_5/{mode}"))
...
def rc4_dir(root: Path) -> Path:
    return root / "data/forward_v5_1_5/rc4"
...
def schema_path(root: Path, config: dict[str, Any]) -> Path:
    p = root / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_5.sql"))
    return p if p.exists() else PROJECT_ROOT / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_5.sql"))
...
def process_start_matches(pid: int | None, recorded: str) -> bool:
    if not pid_alive(pid):
        return False
    # Portable PID-start verification is limited without psutil. The recorded
    # process start time is still saved and stale recovery remains explicit.
...
def lock_is_stale(info: dict[str, Any], config: dict[str, Any], now: datetime | None = None) -> bool:
    ...
    if not pid_alive(pid):
        return True
    if not process_start_matches(pid, str(info.get("process_start_time") or "")):
        return True
    ...
    if not heartbeat_at:
        return True
    try:
        age = ((now or utcnow()).astimezone(timezone.utc) - parse_utc(heartbeat_at)).total_seconds()
    except Exception:
        return True
...
def acquire_monitor_lock(root: Path, mode: str, config: dict[str, Any], run_id: str, command: str = "monitor", recover_stale_lock: bool = False, now: datetime | None = None, config_hash: str = "", code_hash: str = ""):
    ...
    if existing:
        if recover_stale_lock and lock_is_stale(existing, config, now):
            lock.unlink(missing_ok=True)
        else:
            raise RuntimeError(f"monitor lock is already held: {lock}")
    payload = {
...
def frozen_file_records(root: Path, config_path: Path) -> dict[str, dict[str, Any]]:
    ...
    paths = {
        ...
        "core_code_sha256": root / "src/forward_simulation_v5_1_5.py",
        "adapter_code_sha256": root / "src/polymarket_public_adapter_v5_1_5.py",
        "reporting_code_sha256": root / "src/forward_reporting_v5_1_5.py",
        "schema_sha256": schema_path(root, config),
        "preregistration_sha256": root / str(config["paths"].get("preregistration_file", "reports/FORWARD_SIMULATION_V5_1_5_PREREGISTRATION.md")),
        "api_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_5_API_CONTRACT.md",
        "fee_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_5_FEE_CONTRACT.md",
        "settlement_finality_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_5_SETTLEMENT_FINALITY_CONTRACT.md",
    }
    ...
    try:
        ...
        with conn:
            ...
            for key, value in hashes.items():
                set_state(conn, key, value)
            append_audit(conn, FORMAL, "", "formal_started_v5_1_5", {"hashes": hashes}, "info", now)
...
def record_snapshot(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, purpose: str, raw_book: dict[str, Any], source_endpoint: str, now: datetime | None = None, gamma_market: dict[str, Any] | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None) -> tuple[str, dict[str, Any], bool]:
    book = normalize_orderbook(raw_book, sig["token_id"], sig["condition_id"], gamma_market)
    config_path = config_path or (root / "config/forward_simulation_v5_1_5.yaml")
    meta = source_meta(root, config_path, mode, source_endpoint, book["content_hash"])
...
def snapshot_meta(conn: sqlite3.Connection, snapshot_id: str, mode: str) -> dict[str, str]:
    row = conn.execute("SELECT adapter_name,adapter_version,adapter_code_hash,data_source,run_environment,raw_response_hash FROM orderbook_snapshots WHERE snapshot_id=? AND mode=? ORDER BY row_id DESC LIMIT 1", (snapshot_id, mode)).fetchone()
    if not row:
        return {"adapter_name": "", "adapter_version": "", "adapter_code_hash": "", "data_source": "", "run_environment": mode, "raw_response_hash": ""}
...
def process_entry_with_bundle(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, bundle: MarketBundle, now: datetime | None = None) -> dict[str, Any]:
    ...
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, sig, "entry", bundle.raw_book, bundle.book_result.url, now, bundle.gamma_market)
...
def process_exit_with_bundle(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, bundle: MarketBundle, now: datetime | None = None) -> list[dict[str, Any]]:
    ...
    if fee_policy["fee_crosscheck_status"] in {"conflict", "unknown"}:
        ...
        return [{"signal_id": sig["signal_id"], "status": "fee_policy_rejected"}]
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, sig, "exit", bundle.raw_book, bundle.book_result.url, now, bundle.gamma_market)
    validation = validate_token_mapping(dict(sig), bundle.gamma_market, bundle.clob_info, book)
    if not validation["mapping_valid"]:
...
def process_active_token_batch(
    ...
    if not signal_bundles:
        return results
    first_sig, first_gamma, _ = signal_bundles[0]
    snapshot_id, book, inserted = record_snapshot(conn, run_id, mode, first_sig, "monitor", book_result.payload, book_result.url, now, first_gamma.payload, root, config_path)
    if not inserted:
    ...
    for sig, gamma_result, clob_result in ordered_for_entry:
        ...
        if duplicate:
            continue
        buy = consume_buy_levels(ask_levels, dec(state["remaining_entry_usd"]), dec(sig["max_entry_price"]), book["min_order_size"])
        if buy["filled_shares"] <= ZERO:
            conn.execute(
                "INSERT INTO entry_order_state(signal_id,token_id,updated_at_utc,intended_usd,filled_entry_usd,remaining_entry_usd,filled_entry_shares,entry_status,max_entry_price,entry_deadline_utc,last_entry_attempt_at,last_attempt_reason,mode) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    for strategy_id in STRATEGY_IDS:
        ...
        for sig, trigger, position in exit_items:
            ...
            if sell["filled_shares"] <= ZERO:
                continue
            fee_policy = extract_fee_policy(signal_bundles[0][1].payload, signal_bundles[0][2].payload)
            fee_calc = calculate_fee("sell", sell["filled_shares"], sell["vwap"], fee_policy)
            if fee_calc["net_cost_or_proceeds"] is None:
                continue
            fill_id = id_for("exit", {"run_id": run_id, "trigger_id": trigger["trigger_id"], "snapshot_id": snapshot_id, "shares": sell["filled_shares"]})
...
def settle_signal_with_market(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, gamma_market: dict[str, Any], clob_info: dict[str, Any], source_endpoint: str, now: datetime | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None, clob_public_info: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for strategy_id in STRATEGY_IDS:
        ...
        meta = source_meta(root, config_path or (root / "config/forward_simulation_v5_1_5.yaml"), mode, source_endpoint, evidence["raw_response_hash"])
...
def write_resolved_raw_evidence(root: Path, config: dict[str, Any], mode: str, run_id: str, sig: sqlite3.Row, gamma_result: Any, clob_result: Any, now: datetime | None = None, clob_public_result: Any | None = None) -> dict[str, Any]:
    ...
    if mode == LIVE:
        targets.append(rc4_dir(root) / "resolved_market_raw" / name)
...
def monitor_once(root: Path, mode: str, config_path: Path, run_id: str | None = None, adapter: PublicAdapter | None = None, now: datetime | None = None, use_lock: bool = True, recover_stale_lock: bool = False) -> dict[str, Any]:
    config = load_config(config_path)
    now = (now or utcnow()).astimezone(timezone.utc)
    ...
    if use_lock:
        ...
        with acquire_monitor_lock(root, mode, config, rid, command="monitor_once", recover_stale_lock=recover_stale_lock, now=now, config_hash=hashes.get("config_sha256", ""), code_hash=hashes.get("core_code_sha256", "")):
            return monitor_once(root, mode, config_path, rid, adapter=adapter, now=now, use_lock=False)
    db = init_ledger(root, mode, config_path)
        ...
        with conn:
            ...
            for sig in sorted(signals.values(), key=lambda r: (r["created_at_utc"], r["signal_id"])):
                ...
                try:
                    ...
                    state = market_state(gamma_result.payload)
                    ...
                    if state["market_status"] == "active_trading":
                    ...
                    elif state["market_status"] in {"resolution_pending", "active_not_accepting_orders", "disputed", "unknown"}:
                        ...
                        results.extend(settle_signal_with_market(conn, rid, mode, sig, gamma_result.payload, clob_result.payload, gamma_result.url, now, root, config_path, clob_public_result.payload))
                    else:
            ...
            for token_id, bundle_rows in active_by_token.items():
                try:
                    ...
                    results.extend(process_active_token_batch(conn, rid, mode, token_id, bundle_rows, book_result, now, root, config_path))
                except Exception as exc:
...
def run_loop(
    ...
    hashes = current_hashes(root, config_path)
    with acquire_monitor_lock(root, mode, config, rid, command="run_loop", recover_stale_lock=recover_stale_lock, config_hash=hashes.get("config_sha256", ""), code_hash=hashes.get("core_code_sha256", "")):
        try:
            while iterations == 0 or completed < iterations:
                ...
                result = monitor_once(root, mode, config_path, run_id=rid, now=utcnow(), use_lock=False)
...
def audit_integrity(root: Path, mode: str, config_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        checks["proposed_final_settlement"] = conn.execute("SELECT COUNT(*) c FROM settlements WHERE mode=? AND (settlement_status='proposed' OR lower(resolution_status) LIKE '%proposed%' OR lower(uma_status) LIKE '%proposed%')", (mode,)).fetchone()["c"] if False else 0
        ...
        for row in conn.execute("SELECT signal_id,created_at_utc,registered_at_utc FROM signals WHERE mode=?", (mode,)):
            try:
        ...
        for row in conn.execute("SELECT raw_response,raw_response_hash FROM settlements WHERE mode=?", (mode,)):
            raw_response = row["raw_response"]
            if not raw_response:
                checks["missing_settlement_raw_response"] += 1
                continue
            try:
                if content_hash(json.loads(raw_response)) != row["raw_response_hash"]:
...
def fetch_resolved_weather_evidence(root: Path, adapter: PublicAdapter, config: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    rc4_raw = rc4_dir(root) / "resolved_market_raw"
    ...
    for slug in config.get("live_integration", {}).get("resolved_weather_slugs", []):
        try:
            ...
            write_json(rc4_raw / file_name, payload)
            write_json(out_dir / "resolved_weather_evidence.json", payload)
            write_json(rc4_dir(root) / "resolved_market_validation.json", {"generated_at_utc": now_iso(), "validations": validations})
    ...
    write_json(rc4_dir(root) / "resolved_market_validation.json", result)
    return result
...
def live_integration(root: Path, config_path: Path, iterations: int, interval_seconds: Decimal, run_id: str | None = None) -> dict[str, Any]:
    ...
    discovered = discover_weather_markets(adapter, config)
    token_targets: list[dict[str, Any]] = []
    seen_tokens: set[str] = set()
    for market in discovered:
        if len(token_targets) >= 3:
            break
        try:
            slug = str(market.get("slug") or "")
            ...
            if not token_id or token_id in seen_tokens:
                continue
            token_targets.append({"market_slug": slug, "condition_id": condition_id, "token_id": token_id, "outcome": chosen.get("outcome", ""), "market_question": gamma.payload.get("question") or gamma.payload.get("title") or ""})
            seen_tokens.add(token_id)
            write_json(out_dir / "raw_markets" / f"{len(token_targets):02d}_{slug}.json", {"gamma": gamma.payload, "clob": clob.payload, "gamma_endpoint": gamma.url, "clob_endpoint": clob.url})
        except Exception as exc:
    ...
    write_json(rc4_dir(root) / "live_run_manifest.json", manifest)
    return manifest
...
def formal_empty_proof(root: Path, config_path: Path) -> dict[str, Any]:
    ...
    write_json(root / "data/forward_v5_1_5/formal_empty_proof.json", proof)
    write_json(rc4_dir(root) / "formal_empty_proof.json", proof)
    return proof
