#!/usr/bin/env python3
"""SQLite-backed weather forward simulation v5.1.6-RC5.

This module is standalone. It does not import v5, v5.1, v5.1.1, or v5.1.2.
Formal monitor commands use the v5.1.6 public adapter as the only live price
...
try:
    from src.polymarket_public_adapter_v5_1_6 import (
        ADAPTER_NAME,
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
    ...
    from polymarket_public_adapter_v5_1_6 import (
        ADAPTER_NAME,
        ADAPTER_VERSION,
        CLOB_BASE,
        GAMMA_BASE,
...
VERSION = "forward_simulation_v5.1.6-rc5"
SCHEMA_VERSION = "forward_v5_1_6_schema_001"
FORMAL = "formal"
DEMO = "demo"
...
HASH_FILES = {
    "config_sha256": "config/forward_simulation_v5_1_6.yaml",
    "core_code_sha256": "src/forward_simulation_v5_1_6.py",
    "adapter_code_sha256": "src/polymarket_public_adapter_v5_1_6.py",
    "reporting_code_sha256": "src/forward_reporting_v5_1_6.py",
    "schema_sha256": "schemas/forward_simulation_v5_1_6.sql",
    "preregistration_sha256": "reports/FORWARD_SIMULATION_V5_1_6_PREREGISTRATION.md",
    "api_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_6_API_CONTRACT.md",
    "fee_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_6_FEE_CONTRACT.md",
    "settlement_finality_contract_sha256": "reports/FORWARD_SIMULATION_V5_1_6_SETTLEMENT_FINALITY_CONTRACT.md",
}
...
def data_dir(root: Path, mode: str, config: dict[str, Any]) -> Path:
    if mode == LIVE:
        return root / str(config["paths"].get("live_integration_dir", "data/forward_v5_1_6/live_integration"))
    return root / str(config["paths"].get(f"{mode}_data_dir", f"data/forward_v5_1_6/{mode}"))
...
def rc5_dir(root: Path) -> Path:
    return root / "data/forward_v5_1_6/rc5"
...
def schema_path(root: Path, config: dict[str, Any]) -> Path:
    p = root / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_6.sql"))
    return p if p.exists() else PROJECT_ROOT / str(config["paths"].get("schema_file", "schemas/forward_simulation_v5_1_6.sql"))
...
def frozen_file_records(root: Path, config_path: Path) -> dict[str, dict[str, Any]]:
    ...
    paths = {
        ...
        "core_code_sha256": root / "src/forward_simulation_v5_1_6.py",
        "adapter_code_sha256": root / "src/polymarket_public_adapter_v5_1_6.py",
        "reporting_code_sha256": root / "src/forward_reporting_v5_1_6.py",
        "schema_sha256": schema_path(root, config),
        "preregistration_sha256": root / str(config["paths"].get("preregistration_file", "reports/FORWARD_SIMULATION_V5_1_6_PREREGISTRATION.md")),
        "api_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_6_API_CONTRACT.md",
        "fee_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_6_FEE_CONTRACT.md",
        "settlement_finality_contract_sha256": root / "reports/FORWARD_SIMULATION_V5_1_6_SETTLEMENT_FINALITY_CONTRACT.md",
    }
    ...
    try:
        ...
        with conn:
            ...
            for key, value in hashes.items():
                set_state(conn, key, value)
            append_audit(conn, FORMAL, "", "formal_started_v5_1_6", {"hashes": hashes}, "info", now)
...
def record_snapshot(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, purpose: str, raw_book: dict[str, Any], source_endpoint: str, now: datetime | None = None, gamma_market: dict[str, Any] | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None, lock_id: str = "") -> tuple[str, dict[str, Any], bool]:
    ...
    config_path = config_path or (root / "config/forward_simulation_v5_1_6.yaml")
...
def settle_signal_with_market(conn: sqlite3.Connection, run_id: str, mode: str, sig: sqlite3.Row, gamma_market: dict[str, Any], clob_info: dict[str, Any], source_endpoint: str, now: datetime | None = None, root: Path = PROJECT_ROOT, config_path: Path | None = None, clob_public_info: dict[str, Any] | None = None, lock_id: str = "") -> list[dict[str, Any]]:
    ...
    for strategy_id in STRATEGY_IDS:
        ...
        meta = source_meta(root, config_path or (root / "config/forward_simulation_v5_1_6.yaml"), mode, source_endpoint, evidence["raw_response_hash"])
...
def write_resolved_raw_evidence(root: Path, config: dict[str, Any], mode: str, run_id: str, sig: sqlite3.Row, gamma_result: Any, clob_result: Any, now: datetime | None = None, clob_public_result: Any | None = None) -> dict[str, Any]:
    ...
    if mode == LIVE:
        targets.append(rc5_dir(root) / "resolved_market_raw" / name)
...
def fetch_resolved_weather_evidence(root: Path, adapter: PublicAdapter, config: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    rc5_raw = rc5_dir(root) / "resolved_market_raw"
    ...
    for slug in config.get("live_integration", {}).get("resolved_weather_slugs", []):
        try:
            ...
            write_json(rc5_raw / file_name, payload)
            write_json(out_dir / "resolved_weather_evidence.json", payload)
            write_json(rc5_dir(root) / "resolved_market_validation.json", {"generated_at_utc": now_iso(), "validations": validations})
    ...
    write_json(rc5_dir(root) / "resolved_market_validation.json", result)
    return result
...
def live_integration(root: Path, config_path: Path, iterations: int, interval_seconds: Decimal, run_id: str | None = None) -> dict[str, Any]:
    ...
    write_json(rc5_dir(root) / "live_run_manifest.json", manifest)
    write_json(rc5_dir(root) / "real_signal_to_fill_validation.json", signal_to_fill)
    return manifest
...
def formal_empty_proof(root: Path, config_path: Path) -> dict[str, Any]:
    ...
    write_json(root / "data/forward_v5_1_6/formal_empty_proof.json", proof)
    write_json(rc5_dir(root) / "formal_empty_proof.json", proof)
    return proof
